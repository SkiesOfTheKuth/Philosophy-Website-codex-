...
class ApiClient {
    constructor(basePath = '/api', options = {}) {
    ...
    async post(endpoint, payload) {
        ...
        const timeoutId = supportsAbort ? window.setTimeout(() => controller.abort(), this.timeoutMs) : null;
        ...
        try {
            ...
            if (supportsAbort && error.name === 'AbortError') {
                throw new ApiError('Request timed out. Please try again in a moment.', 408);
            }
            if (error instanceof TypeError) {
                throw new ApiError('Network connection appears offline. Check your connection and try again.', 0);
            }
            throw error;
        } finally {
            if (timeoutId !== null) {
...
class FormValidator {
    constructor(form) {
        ...
        this.fields = Array.from(form.querySelectorAll('input, textarea'))
            .filter(field => field.type !== 'hidden' && !field.hasAttribute('data-ignore-validation'));
    }
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            if (error instanceof ValidationApiError) {
                ...
                let message = error.message || 'We could not submit your request just yet.';
                if (error.status === 429) {
                    message = 'We’re receiving lots of requests. Please wait a minute and try again.';
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            if (error instanceof ValidationApiError && error.errors?.email) {
                ...
                let message = error.message || 'Unable to subscribe right now. Please try again later.';
                if (error.status === 429) {
                    message = 'You have reached the subscription limit for now. Retry shortly!';
                } else if (error.status === 408) {
                    message = 'The request is taking too long. Please check your connection and retry.';
                } else if (error.status === 0) {
                    message = 'It looks like you are offline. Reconnect and try again.';
                }
                this.updateFeedback(message, false);
            } else {
                console.error(error);
...
class AuroraApp {
    constructor() {
        ...
        this.apiClient = new ApiClient('/api', { timeoutMs: 12000 });
    }
