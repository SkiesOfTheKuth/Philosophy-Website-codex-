...
## [2025-09-30] Resilience and ops hardening
- Added optional dependency loader so the API can warn and continue when compression/HPP/rate limiting modules are unavailable.
- Introduced structured JSON logging, graceful shutdown handling, and surfaced dependency/storage health via `/api/health`.
- Replaced `nanoid` with native `crypto.randomUUID` to remove ESM friction in test environments.
- Authored operations, architecture, security, and migration guides while updating the README and continuity guide to reference them.
- Established a GitHub Actions CI workflow running `npm test` on pushes and pull requests.
