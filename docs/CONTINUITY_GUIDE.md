...
├─ utils/          Async wrapper and HTTP error helpers
...
1. `server/app.js` configures Express with Helmet, compression, HPP, rate limiting, body parsing, optional CORS, static delivery, and centralized error handling.
...
- Observability is currently limited to console logging and the health endpoint; future iterations should add structured logging and metrics.
...
2. **Observability:** Add structured logging, tracing, and analytics with consent management. Consider integrating with Datadog or OpenTelemetry exporters.
...
2. **Observability Layer (High):** Introduce structured logging and error tracking (pino + Sentry). Expose metrics via `/api/health` or a new `/api/status` endpoint.
