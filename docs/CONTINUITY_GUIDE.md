...
- `server/app.js` configures the Express application with security headers, optional CORS controls, structured logging, API routing, static asset hosting, and centralized error handling.
- `server/routes/` contains REST endpoints for `/api/health`, `/api/contact`, and `/api/newsletter`.
- `server/services/leadService.js` keeps in-memory records of contacts/newsletter signups (replace with a persistent store before production go-live).
...
## Backend Overview
- **Framework:** Express 4 hardened with Helmet, optional CORS allow-lists, JSON/urlencoded body parsing, and combined logging via Morgan.
- **Endpoints:**
  - `POST /api/contact` – Accepts `{ name, email, company, message }`, validates via Zod, stores the payload in the in-memory `leadService`, and returns a 201 with an identifier/timestamp. Responds with a 422 status and field-level errors when validation fails.
  - `POST /api/newsletter` – Accepts `{ email }`, applies identical validation patterns, and stores subscribers.
  - `GET /api/health` – Lightweight status probe used for uptime checks.
...
- **Error handling:** Central middleware normalizes error payloads. Validation errors surface structured `{ errors: { field: [messages] } }` data; unexpected exceptions log server-side and return a generic 500 response to callers.
- **Testing:** Jest + Supertest cover happy/validation paths for both form endpoints and the health probe (see `server/tests/api.test.js`).

## Tooling & Testing
- Install dependencies with `npm install`.
- Run automated API tests via `npm test` (Jest executes against the Express app with in-memory stores reset after each case).
...
- **Analytics & integrations:** Embed consent-aware analytics (e.g., Plausible) and replace the in-memory lead service with durable CRM or marketing automation integration.
